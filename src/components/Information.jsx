...
function Information(props) {
    ...
    return (
        <main className='flex-1 p-4 flex flex-col gap-3 text-center sm:gap-4 justify-center pb-20 w-full max-w-fit mx-auto mt-10'>
            <h1 className='font-semibold text-3xl sm:text-5xl md:text-6xl'>Your <span className='text-purple-400 bold nowrap'>{tab}</span></h1>
            <div className='grid grid-cols-2 sm:mx-auto bg-white rounded overflow-hidden items-center p-1 purpleShadow border-[3px] border-solid border-purple-400'>
                <button className={'px-4 rounded duration-200 py-1 ' + (tab === 'Transcription' ? ' bg-purple-300 text-white' : ' text-purple-400 hover:text-purple-600')}
                    onClick={handleTabChange}
                >Transcription</button>
                <button className={'px-4 rounded duration-200 py-1 ' + (tab === 'Translation' ? ' bg-purple-300 text-white' : ' text-purple-400 hover:text-purple-600')}
                    onClick={handleTabChange}
                >Translation</button>
            </div>
